<?php

/**
 * Template Name: Slick版型 (ex淨水器)
 * @package slick slide
 * 需要在 /assets/js/admin.js 調整後台選單
 */


// Exit if accessed directly.
defined('ABSPATH') || exit;

get_header();

/**
 * 高澄嚴選
 */



[
    'brand_name'    => $brand_name,
    'brand_image' => $brand_image
] = rwmb_meta('brand_data_left');
$brand_image = wp_get_attachment_image_url($brand_image, 'full');

[
    'title_name'    => $title_name,
    'fb_link'       => $fb_link,
    'site_link'     => $site_link,
    'brand_content' => $brand_content
] = rwmb_meta('brand_data_right');


/**
 * Slick slide
 */

$slide_area = rwmb_meta('slide_area');

echo do_shortcode( '[ht_banner]' );

?>


<section class="mt-5r mb-3r">
    <div class="container">
        <div class="row">
            <div class="col-lg-12">
                <div class="lc-block">
                    <div editable="rich">
                        <h1 class="mb-3r"><?= $brand_name ?></h1>
                    </div>
                </div><!-- /lc-block -->
            </div><!-- /col -->
        </div>
        <div class="row">
            <div class="col-lg-6">
                <div class="lc-block mb-2r"><a href="<?= $brand_image ?>"><img class="w-100" src="<?= $brand_image ?>"></a></div><!-- /lc-block -->
            </div><!-- /col -->
            <div class="col-lg-6">
                <div class="lc-block position-relative">
                    <div editable="rich">
                        <div class="border-gray-900 ps-2r mb-2r" style="border-left:1rem solid">
                            <h2 class="text-gray-900"><?= $title_name ?></h2>
                        </div>
                        <div style="text-align:justify;">
                            <?= wpautop($brand_content); ?>
                        </div>
                    </div>
                    <div class="position-absolute end-0" style="top:-1rem;">
                        <?php if (!empty($fb_link)) : ?>
                            <a href="<?= $fb_link ?>" target="_blank" class="nolightbox me-2" rel="noopener"><img src="data:image/png;base64," width="34" height="34"></a>
                        <?php endif; ?>
                        <?php if (!empty($site_link)) : ?>
                            <a href="<?= $site_link ?>" target="_blank" rel="noopener" class="nolightbox"><img src="data:image/png;base64," width="34" height="34"></a>
                        <?php endif; ?>
                    </div>
                </div><!-- /lc-block -->
            </div><!-- /col -->

            <div class="col-12 mt-2r">
                <hr class="opacity-1 text-gray-900">
            </div>
        </div>
    </div>
</section>

<?php foreach ($slide_area as $key => $slide_single_area) :
    [
        'slide_title' => $slide_title,
        'slide_content' => $slide_content,
        'btn_text' => $btn_text,
        'btn_link' => $btn_link,
        'in_new_window' => $in_new_window,
    ] = $slide_single_area['slide_left'];

    [
        'slide' => $slides,
    ] = $slide_single_area['slide_right'];
    //var_dump($slide_single_area['slide_right']);
?>

    <section class="mb-3r">
        <div class="container">
            <div class="row">
                <div class="col-lg-6 mb-3r">
                <?php if(!empty($slide_title)): ?>
                        <div class="border-gray-900 ps-2r mb-2r" style="border-left:1rem solid">
                            <h2 class="text-gray-900"><?= $slide_title ?></h2>
                        </div>
                        <?php endif; ?>
                    <div class="lc-block">
                        <div editable="rich">
                            <?= wpautop($slide_content) ?>
                        </div>
                    </div>
                    <?php if (!empty($btn_text)) : ?>
                        <div class="lc-block text-end mt-2r"><a class="btn btn-primary" href="<?= $btn_link ?>" role="button" <?= ($in_new_window) ? 'target="_blank"' : '' ?> rel="noopener"><?= $btn_text ?></a></div><!-- /lc-block -->
                    <?php endif; ?>
                </div><!-- /col -->
                <div class="col-lg-6">
                    <div class="lc-block">
                        <div class="ht-slide ratio-1x1">
                            <?php foreach ($slides as $key => $slide) :
                                $slide = wp_get_attachment_image_url($slide, 'full');
                            ?>
                                <div><a href="<?= $slide ?>"><img src="<?= $slide ?>" class=""></a></div>
                            <?php endforeach; ?>
                        </div>

                    </div>
                </div><!-- /col -->
                <div class="col-12 mt-2r">
                    <hr class="opacity-1 text-gray-900">
                </div>
            </div>
        </div>
    </section>


<?php endforeach; ?>
<script>
    jQuery('.ht-slide').slick({
        dots: true,
        arrows: false
    });
</script>




<?php get_footer();
