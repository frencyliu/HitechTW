<?php

/**
 * Template Name: 人氣設計版型 (ex室內設計)
 * 需要在 /assets/js/admin.js 調整後台選單
 */


// Exit if accessed directly.
defined('ABSPATH') || exit;

get_header();

/**
 * 高澄嚴選
 */



[
    'brand_name'    => $brand_name,
    'brand_image' => $brand_image
] = rwmb_meta('brand_data_left');
$brand_image = wp_get_attachment_image_url($brand_image, 'full');

[
    'title_name'    => $title_name,
    'fb_link'       => $fb_link,
    'site_link'     => $site_link,
    'brand_content' => $brand_content
] = rwmb_meta('brand_data_right');


/**
 * 人氣設計 portfolio
 */

$portfolio_title = rwmb_meta('portfolio_title');
$portfolio_ht_company = rwmb_meta('portfolio_ht_company');
$portfolio_number = rwmb_meta('portfolio_number') ?: 8;



//var_dump($portfolio_number);
echo do_shortcode( '[ht_banner]' );
?>


<section class="mt-5r mb-3r">
    <div class="container">
        <div class="row">
            <div class="col-lg-12">
                <div class="lc-block">
                    <div editable="rich">
                        <h1 class="mb-3r"><?= $brand_name ?></h1>
                    </div>
                </div><!-- /lc-block -->
            </div><!-- /col -->
        </div>
        <div class="row">
            <div class="col-lg-6">
                <div class="lc-block mb-2r"><a href="<?= $brand_image ?>"><img class="w-100" src="<?= $brand_image ?>"></a></div><!-- /lc-block -->
            </div><!-- /col -->
            <div class="col-lg-6">
                <div class="lc-block position-relative">
                    <div editable="rich">
                    <?php if(!empty($title_name)): ?>
                        <div class="border-gray-900 ps-2r mb-2r" style="border-left:1rem solid">
                            <h2 class="text-gray-900"><?= $title_name ?></h2>
                        </div>
                        <?php endif; ?>
                        <div style="text-align:justify;">
                            <?= wpautop($brand_content); ?>
                        </div>
                    </div>
                    <div class="position-absolute end-0" style="top:-1rem;">
                        <?php if (!empty($fb_link)) : ?>
                            <a href="<?= $fb_link ?>" target="_blank" class="nolightbox me-2" rel="noopener"><img src="data:image/png;base64," width="34" height="34"></a>
                        <?php endif; ?>
                        <?php if (!empty($site_link)) : ?>
                            <a href="<?= $site_link ?>" target="_blank" rel="noopener" class="nolightbox"><img src="data:image/png;base64," width="34" height="34"></a>
                        <?php endif; ?>
                    </div>
                </div><!-- /lc-block -->
            </div><!-- /col -->

            <div class="col-12 mt-2r">
                <hr class="opacity-1 text-gray-900">
            </div>
        </div>
    </div>
</section>



<section class="mb-3r">
<div class="container">
	<div class="row">
        <?php if(!empty($portfolio_title)): ?>
		<div class="col-lg-12 mb-3r">
			<div class="lc-block border-gray-900 ps-2r" style="border-left:1rem solid">
				<h2 class="text-gray-900"><?= $portfolio_title; ?></h2>
			</div>
		</div><!-- /col -->
        <?php endif; ?>
		<div class="col-lg-12">
			<div class="lc-block">
				<div lc-helper="posts-loop" class="live-shortcode">

                <?php
                    $shortcode = '[lc_get_posts post_type="ht-select" posts_per_page="' . $portfolio_number . '" tax_query="ht-company=' . $portfolio_ht_company->term_id . '" output_view="yc_get_posts_flip_card_view" output_number_of_columns="4" output_article_class="shadow ratio ratio-1x1" output_excerpt_text="案例介紹" output_featured_image_class="card-img-top" ]';

                    echo do_shortcode($shortcode);
                ?>
                </div>
			</div>
		</div><!-- /col -->
	</div>
</div>
</section>








<?php get_footer();
